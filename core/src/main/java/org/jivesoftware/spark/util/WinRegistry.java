...
public class WinRegistry {
    ...
    private static int deleteKey(Preferences root, int hkey, String key) throws IllegalArgumentException, IllegalAccessException,
        ...
        return (int) (Integer) regDeleteKey.invoke(root, new Object[]{hkey, toCstr(key)});
    }
	...
	private static Map<String, String> readStringValues(Preferences root,
		...
		for (int index = 0; index < count; index++) {
			...
			String value = readString(hkey, key, Arrays.toString(name));
			results.put(Arrays.toString(name).trim(), value);
		}
		regCloseKey.invoke(root, handles[0]);
		return results;
	}
	...
	private static List<String> readStringSubKeys(Preferences root, int hkey,
		...
		for (int index = 0; index < count; index++) {
			...
			results.add(Arrays.toString(name).trim());
		}
		regCloseKey.invoke(root, handles[0]);
		return results;
	}
