...
//    private final Vector<CaptureDeviceInfo2> vectorPlaybackDevices = new Vector<>();
    ...
    private String convertSysString(String src) {
        String res = new String(src.getBytes(StandardCharsets.ISO_8859_1), Charset.defaultCharset());
        return res;
    }
...
/*
        for (AudioSystem system : vectorAudioSystem) {
            if (system.getLocatorProtocol().equals(selectedAudioSystem)) {
                Log.debug("setAudioSystem:" + system);
        ...
        Log.debug("AudioSystem:" + ((MediaServiceImpl) LibJitsi.getMediaService()).getDeviceConfiguration().getAudioSystem());
*/
    }
