    ...
    private void setUpMessageListener() {
        ...
        _msgEvnt = new MessageEventListener() {
            ...
            public void receivingMessage(MessageBuilder message) {
                ...
                if (_OtrEnabled) {
                    try {
                        String decoded = _mySession.transformReceiving(msgBody);
                        // if the message wasn't encrypted, then the decoded will be the same string so compare by a reference
                        //noinspection StringEquality
                        if (decoded != msgBody) {
                            clearMsgBody(message); // clear the body of the original encrypted message
                            if (decoded != null) {
                        ...
                        clearMsgBody(message);
                        _chatRoom.getTranscriptWindow().insertNotificationMessage(OTRResources.getString("otr.failed.to.decode") + " " + e.getMessage(), ChatManager.ERROR_COLOR);
            ...
            public void sessionStatusChanged(SessionID sessionID) {
                ...
                if (_mySession.getSessionStatus() == SessionStatus.ENCRYPTED) {
                    ...
                    if (!keyManager.isVerified(_mySessionID)) {
                        ...
                        if (JOptionPane.showConfirmDialog(_otrButton, dialogMessage, dialogTitle, YES_NO_OPTION, WARNING_MESSAGE) == YES_OPTION) {
                            if (JOptionPane.showConfirmDialog(_otrButton, OTRResources.getString("otr.question.authorize"), dialogTitle, YES_NO_OPTION, WARNING_MESSAGE) == YES_OPTION) {
                                OTRManager.getInstance().authenticateUser(JidCreate.entityBareFromOrNull(_mySessionID.getUserID()));
                            } else {
